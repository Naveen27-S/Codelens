...
def delete_dashboard_history_item(user_id: int, item_id: str) -> bool:
    ...
    try:
        ...
        return False
    result = mongo_db["dashboard_history"].delete_one({"_id": oid, "user_id": user_id})
...
def clear_dashboard_history(user_id: int) -> int:
    ...
    if mongo_db is None:
        return 0
    result = mongo_db["dashboard_history"].delete_many({"user_id": user_id})
