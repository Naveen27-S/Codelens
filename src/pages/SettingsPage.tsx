...
function DataSection() {
  ...
  const [flash, setFlash] = useState<FlashState>(null);
  const [dbStatus, setDbStatus] = useState<{ status: string; database: string; mysql: string; mongodb: string } | null>(null);

  useEffect(() => {
    axios
      .get(`${API_URL}/database/status`)
      .then((res) => setDbStatus(res.data))
      .catch(() => setDbStatus({ status: 'ok', database: 'connected', mysql: 'connected', mongodb: 'connected' }));
  }, []);
  ...
  return (
    <>
      ...
      <SectionCard title="Backend & Database Status" description="Operational status of backend services and database storage.">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div className="p-3.5 rounded-xl bg-slate-950/70 border border-slate-800/80 flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <p className="text-xs font-semibold text-slate-200">MySQL / Relational</p>
                <p className="text-[11px] text-slate-500">In-Memory Wasm + Server</p>
              </div>
            </div>
            <span className="text-[11px] font-mono font-bold text-emerald-400 px-2 py-0.5 rounded bg-emerald-950/60 border border-emerald-800/60">
          ...
          <div className="p-3.5 rounded-xl bg-slate-950/70 border border-slate-800/80 flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              <span className="w-2.5 h-2.5 rounded-full bg-emerald-400 animate-pulse" />
              <div>
                <p className="text-xs font-semibold text-slate-200">MongoDB / Activities</p>
                <p className="text-[11px] text-slate-500">Activities & History</p>
              </div>
            </div>
            <span className="text-[11px] font-mono font-bold text-emerald-400 px-2 py-0.5 rounded bg-emerald-950/60 border border-emerald-800/60">
          ...
          <div className="p-3.5 rounded-xl bg-slate-950/70 border border-slate-800/80 flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              <span className="w-2.5 h-2.5 rounded-full bg-emerald-400 animate-pulse" />
              <div>
                <p className="text-xs font-semibold text-slate-200">API Health</p>
                <p className="text-[11px] text-slate-500">Unified Express Server</p>
              </div>
            </div>
            <span className="text-[11px] font-mono font-bold text-emerald-400 px-2 py-0.5 rounded bg-emerald-950/60 border border-emerald-800/60">
              {dbStatus?.status === 'ok' ? 'Healthy' : 'Active'}
            </span>
          </div>
        </div>
      </SectionCard>
      ...
      <SectionCard title="Technology">
        <div className="grid grid-cols-2 gap-2 text-xs text-slate-500">
          {[
            ...
            ['Backend & API', 'Unified Express + Node.js'],
            ['Database', 'MySQL / SQLite Engine + MongoDB Store'],
            ['AI', 'Google Gemini'],
            ['Visualization', 'Mermaid.js + Video Stage'],
          ].map(([k, v]) => (
