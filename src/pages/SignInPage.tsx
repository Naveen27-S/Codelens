        ...
        <div className="bg-slate-900/60 backdrop-blur-xl border border-slate-800/60 rounded-2xl p-8 shadow-2xl shadow-black/40">
          ...
          <form onSubmit={handleSubmit} noValidate className="space-y-4">
            ...
            <button
              ...
              disabled={loading}
              onClick={async () => {
                setForm({ email: 'demo@codelens.ai', password: 'password123' });
                setLoading(true);
                setErrors({});
                try {
                  await login('demo@codelens.ai', 'password123');
                  navigate('/editor');
                } catch {
              ...
              className="w-full py-2.5 mt-2 bg-slate-800/80 hover:bg-slate-700/80 border border-slate-700/70 text-slate-300 hover:text-white text-xs font-medium rounded-xl transition-all flex items-center justify-center gap-2"
            >
              Sign in with Demo Account (demo@codelens.ai)
            </button>
          </form>
